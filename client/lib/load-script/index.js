...
function _createScriptElement( url, onload = _handleRequestSuccess, onerror = _handleRequestError ) {
	...
	script.async = true;
	script.onload = onload.bind( script, url );
	script.onerror = onerror.bind( script, url );
	return script;
}
...
function _handleRequestSuccess( url, executeCallbacks = _executeCallbacks ) {
	debug( `Handling successful request for ${ url }` );
	executeCallbacks( url );
	this.onload = null;
}

function _handleRequestError( url, executeCallbacks = _executeCallbacks ) {
